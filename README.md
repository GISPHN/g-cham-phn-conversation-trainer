...
MVP 0.6.0 では、保健師がトレーニング目的に応じて対象者を選べるよう、開始前に以下を指定できます。

- ケースレベル: 初級 / 標準 / 上級
- 支援区分: 動機付け支援 / 積極的支援
- 開始時の意思決定状態: まだ考えていない / 迷いがある / 検討している / やってみようかと考えている

指定条件から教育用の初期 ConversationState を構成し、JMED-Personas の職業、生活習慣、経済的制約、家族・社会参加、健診歴、代表発話等を用いて候補をランキングします。完全一致ラベルをJMED-Personasから読み取っているわけではなく、難易度・支援区分・意思決定状態は本アプリ側の教育用設定です。

「この条件で対象者を選んで開始」では指定条件に適した候補群から対象者を選択し、「同じ条件で別の対象者」では現在のJMED dataset IDを除外して別の候補を選びます。「完全ランダムで開始」では3条件自体をランダム化します。

開始時に `self_selected_goal` は選択できません。これは面接過程で対象者自身が目標を選択した場合に到達する状態として扱います。
